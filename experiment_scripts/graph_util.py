...
def plotLine(title, x_axis, y_axis, save, data, points, legend='upper left', xleftlim=0, xrightlim=0, yleftlim=0, yrightlim=0,
    ...
    try:
        ...
        for tup in data:
            ...
            rows = data.shape[0]
            if points:
                plt.plot(data[:, x], data[:, y], mark[i],
                         linewidth=2, label=lab, markersize=4)
            else:
...
def plotStackedBars(title, barNames, datasetName, yAxis, data_, showvalues, save, dataStd=[], legend='upper left', ylim=0, black=True,
    ...
    try:
        ...
        for tup in data_:
            data_path = tup[0]
            values = np.atleast_2d(np.loadtxt(
                data_path, dtype="string", delimiter=" "))
            index = tup[1]
            print(index)
        ...
        for d in range(0, len(data[0])):
            if (len(dataStd) == 0):
                ...
                for j in range(0, len(stacked[0])):
                    ...
                    if not black:
                        rects = ax.bar(
                            ind + offset, spl, width, bottom=bot, color=colours[k], label=datasetName[k])
                    else:
                        rects = ax.bar(
                            ind + offset, spl, width, bottom=bot, color=colours[k], label=datasetName[k])
                    prev_spl = spl
...
def plotBars(title, barNames, datasetName, yAxis, data_, showvalues, save, dataStd_=[], legend='upper left', ylim=0, black=True,
    ...
    for d in datasetName:
        ...
        if (len(dataStd) == 0):
            if not black:
                rects = ax.bar(
                    ind + offset, data[i], width, bottom=0, color=colours[i], label=datasetName[i])
            else:
                rects = ax.bar(ind + offset, data[i], width, bottom=0,
                               color=colours[i], hatch=patterns[i], label=datasetName[i])

        else:
            if not black:
                print("Not black")
                rects = ax.bar(ind + offset, data[i], width, bottom=0, yerr=(
                    dataStd[i]), color=colours[i], label=datasetName[i])
            else:
                rects = ax.bar(ind + offset, data[i], width, bottom=0, yerr=(
                    dataStd[i]), color=colours[i], hatch=patterns[i], label=datasetName[i])
        i = i + 1
    ...
    try:
        ...
        for d in datasetName:
            if (len(dataStd) == 0):
                ...
                if not black:
                    rects = ax.bar(ind + offset, data[i], width, bottom=0, color=colours[i], yerr=tuple(
                        dataStd[i]), label=datasetName[i])
                else:
                    rects = ax.bar(
                        ind + offset, data[i], width, bottom=0, yerr=tuple(dataStd[i]), label=datasetName[i])
            lines.append(rects)
            i = i + 1
